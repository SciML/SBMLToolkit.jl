...
SPECIES3 = SBML.Species(name = "s3", compartment = "c1", initial_amount = 1.0,
                        substance_units = "substance", only_substance_units = false,
                        constant = true)
...
REACTION1 = SBML.Reaction(reactants = Dict(),
                          ...
                          reversible = false)
REACTION2 = SBML.Reaction(reactants = Dict("s2" => 1),
                          products = Dict(),
                          kinetic_parameters = kinetic_params,
                          kinetic_math = KINETICMATH2,
                          reversible = false)
REACTION3 = SBML.Reaction(reactants = Dict("s1" => 1),
                          products = Dict(),
                          kinetic_parameters = kinetic_params,
...
MODEL3 = SBML.Model(parameters = Dict("k1" => PARAM1),
                    compartments = Dict("c1" => COMP1),
                    species = Dict("s1" => SPECIES1),
...
reac = SBML.Reaction(reactants = Dict("s1" => 1),
                     ...
                     kinetic_math = km,
                     reversible = false)
mod = SBML.Model(parameters = Dict("k1" => PARAM1),
                 compartments = Dict("c1" => COMP1),
                 species = Dict("s1" => SPECIES1),
